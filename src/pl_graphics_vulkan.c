...
pl_create_shader(plGraphics* ptGraphics, const plShaderDesc* ptDesc, plShader* ptShaderOut)
{
    ...
    VkDescriptorSetLayout* atDescriptorSetLayouts = malloc(ptDesc->uBindGroupLayoutCount * sizeof(VkDescriptorSetLayout));
    ...
    if(ptDesc->tMeshFormatFlags0 & PL_MESH_FORMAT_FLAG_HAS_POSITION)
    {
    ...
    if(ptDesc->tMeshFormatFlags0 & PL_MESH_FORMAT_FLAG_HAS_NORMAL)
    {
        const VkVertexInputAttributeDescription tAttributeDescription =
        {
            .binding  = 0,
    ...
    if(ptDesc->tMeshFormatFlags0 & PL_MESH_FORMAT_FLAG_HAS_TANGENT)
    {
        const VkVertexInputAttributeDescription tAttributeDescription =
        {
            .binding  = 0,
    ...
    const VkPipelineColorBlendAttachmentState tColorBlendAttachment = pl__get_blend_state(ptDesc->tGraphicsState.tBlendMode);
    ...
    VkPipelineDepthStencilStateCreateInfo tDepthStencil = {
        ...
        .depthTestEnable       = ptDesc->tGraphicsState.bDepthEnabled ? VK_TRUE : VK_FALSE,
        .depthWriteEnable      = ptDesc->tGraphicsState.bDepthEnabled ? VK_TRUE : VK_FALSE,
    ...
    const VkPipelineRasterizationStateCreateInfo tRasterizer = {
        ...
        .lineWidth               = 1.0f,
        .cullMode                = ptDesc->tGraphicsState.tCullMode,
        .frontFace               = ptDesc->tGraphicsState.bFrontFaceClockWise ? VK_FRONT_FACE_CLOCKWISE : VK_FRONT_FACE_COUNTER_CLOCKWISE,
        .depthBiasEnable         = ptDesc->tGraphicsState.fDepthBias > 0.0f ? VK_TRUE : VK_FALSE
    };
...
void
pl_update_bind_group(plGraphics* ptGraphics, plBindGroup* ptGroup, plBindUpdate* ptBindings)
{
    ...
    pl_sb_resize(sbtWrites, ptBindings->uBufferCount + ptBindings->uTextureCount);
    pl_sb_resize(sbtBufferDescInfos, ptBindings->uBufferCount);
    pl_sb_resize(sbtImageDescInfos, ptBindings->uTextureCount);

    uint32_t uCurrentWrite = 0;
    for(uint32_t i = 0 ; i < ptBindings->uBufferCount; i++)
    {

        sbtBufferDescInfos[i].buffer = ptBindings->aBuffers[i].tBuffer.tBuffer;
        sbtBufferDescInfos[i].offset = ptBindings->aBuffers[i].szOffset;
        sbtBufferDescInfos[i].range  = ptBindings->aBuffers[i].szSize;
    ...
    for(uint32_t i = 0 ; i < ptBindings->uTextureCount; i++)
    {

        sbtImageDescInfos[i].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        sbtImageDescInfos[i].imageView   = ptBindings->aTextures[i].tTexture.tImageView;
        sbtImageDescInfos[i].sampler     = ptBindings->aTextures[i].tSampler;
